...
export function ImportBackupScreen({
  navigation,
}: NativeStackScreenProps<RootStackParamList, "ImportBackup">) {
  ...
  const importBackupButtonTranslateX = useSharedValue(0);
  const importBackupButtonTranslateXAnimatedStyle = useAnimatedStyle(() => {
    return {
      transform: [
        {
          translateX: importBackupButtonTranslateX.value,
        },
      ],
    };
  });

  const shakeImportBackupButton = () => {
    const offset = 2;
    const duration = 50;
    importBackupButtonTranslateX.value = withSequence(
      withTiming(offset, {
        duration: duration,
        easing: EASING,
      }),
    ...
    if (password) {
      try {
        ...
        if (err instanceof InvalidKeyOrCorruptedBackupError) {
          ...
          msg = lang.importBackupScreen.incorrectPasswordOrCorruptedBackupError;
        } else {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {lang.importBackupScreen.cautionFirstLine}
              {"\n"}
              {lang.importBackupScreen.cautionSecondLine}
            </Text>
          </View>
          <View
            style={{
